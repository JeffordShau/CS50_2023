...
person *create_family(int generations)
{
    ...
    if (generations > 1)
    {
        ...
        int parent1allele = rand() % 2;
        int parent2allele = rand() % 2;
        current->alleles[0] = current->parents[0]->alleles[parent1allele];
        current->alleles[1] = current->parents[1]->alleles[parent2allele];
    }
